...
import CompanyImageForm from "./company-image-form";

interface CompanyImageModalProps {
  isOpen: boolean;
  onClose: () => void;
  companyId?: string
}
...
export default function CompanyImageModal({ isOpen, onClose, companyId }: CompanyImageModalProps) {
  ...
  return (
    <div onClick={handleClickOutside} className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-sm border border-slate-800 w-full max-w-xl">
        ...
        <CompanyImageForm onSuccess={handleClose} companyId={companyId!} />
      </div>
    </div>
  )
}