...
export default function UserImageForm({ onSuccess }: UserImageFormProps) {
  const { uploadUserImage, error, isLoading } = useAuthStore();
  ...
  return (
    <form onSubmit={handleUpload} className="flex flex-col gap-6 m-4">
      ...
      >
        {preview ? (
          <>
            <img src={preview} alt="preview" className="w-full max-h-122.5 object-cover rounded-lg" />
            <button
              type="button"
